...
const RemoveSong = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const updatedSongs = songs.filter(song => !selectedSongs.includes(song.songId));
  ...
  return (
    <form className='flex flex-col w-[400px] max-md:w-[90%]' onSubmit={handleSubmit}>
      <h2 className=' text-xl text-textcolor font-semibold mt-5 mb-2'>Select Playlist:</h2>
      <input
        type="text"
        value={searchTerm}
      ...
      {selectedPlaylistId && (
        <>
          ...
          >
            ...
            />
            <label className='font-bold text-xl pr-3 text-slate-900'>Select All</label>
          </div>
        </div>
          ...
          {songs.map((song, index) => (
            ...
            >
              <input
                ...
                onClick={(e) => e.stopPropagation()}  // Prevents the checkbox click from triggering the div click
              />
              <label className=' p-3 w-full text-slate-900 font-bold text-xl bg-slate-400 rounded-r-md'>{song.songId}</label>
            </div>
          ))}
