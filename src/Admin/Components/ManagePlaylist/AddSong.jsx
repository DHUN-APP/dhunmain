    ...
    try {
      ...
      songIds.forEach((songId, index) => {
        if (newSongIdMap.has(songId)) {
          duplicateWithinNewSongs.push(`Song ID ${songId} is duplicated in the submission.`);
        }
      ...
      {selectedPlaylistId &&  (
        <>
          ...
          {songIds.map((songId, index) => (
            <div key={index} className='mb-4 flex items-center justify-center gap-3'>
              <h2 className='text-xl text-textcolor font-semibold text-nowrap'>Song : {index + 1}</h2>
              <input
                type="text"
                value={songId}
