...
const RemoveSong = () => {
  ...
  const handleSuggestionClick = (item, type) => {
    if (type === 'artist') {
      ...
      setSelectedSong(item);  // Ensure item is in the format you expect
      setSongSuggestions([]);
      setSongSearchTerm(item.songName); // Clear the search term for better user experience
    }
  };
  ...
  const handleRemoveSong = async () => {
    ...
    try {
      ...
      const updatedSongs = existingSongs.filter(song =>
      ...
      {selectedSong && (
        <>
          <div className='mb-4 flex flex-col items-center justify-center gap-3'>
            ...
            <p className='w-full p-2 bg-slate-600 text-lg text-white font-semibold rounded-md'>{selectedSong.songName}</p>
            <h2 className="text-xl text-textcolor font-semibold w-full">Song ID :</h2>
            <p className='w-full p-2 bg-slate-600 text-lg text-white font-semibold rounded-md'>{selectedSong.songId}</p>
            <h2 className="text-xl text-textcolor font-semibold w-full">Cover Image :</h2>
            <p className='w-full p-2 bg-slate-600 text-lg text-white font-semibold rounded-md'>{selectedSong.coverImgUrl}</p>
            <h2 className="text-xl text-textcolor font-semibold w-full">Song URL :</h2>
            <p className='w-full p-2 bg-slate-600 text-lg text-white font-semibold rounded-md'>{selectedSong.songUrl}</p>
          </div>
          <button
            type="button"
