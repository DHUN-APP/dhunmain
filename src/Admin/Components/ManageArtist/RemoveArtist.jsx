...
const RemoveArtist = () => {
  ...
  const handleRemoveArtist = async () => {
    setLoading(true); // Set loading to true when starting the remove

    try {
      // Proceed with the removal using only the selectedArtistId
      const docRef = doc(db, 'artists', selectedArtistId);
      await deleteDoc(docRef);

      // Reset the form fields and states after a successful removal
      setSearchTerm('');
      setSelectedArtistId('');
      setArtistName('');
      setPhotoURL('');
    ...
    <div className='flex flex-col w-[400px] max-md:w-[90%] overflow-y-scroll scrollbar-hide'>
      ...
      {selectedArtistId && (
        <>
          ...
          <p className='w-full p-2 bg-slate-600 text-lg text-white font-semibold rounded-md'>{artistName}</p>
          <h2 className='text-xl text-textcolor font-semibold mt-5 mb-2'>Artist ID:</h2>
          <p className='w-full p-2 bg-slate-600 text-lg text-white font-semibold rounded-md'>{artistId}</p>
          <h2 className='text-xl text-textcolor font-semibold mt-5 mb-2'>Photo URL:</h2>
          <p className='w-full p-2 bg-slate-600 text-lg text-white font-semibold rounded-md'>{photoURL}</p>
          <h2 className='text-xl text-textcolor font-semibold mt-5 mb-2'>Artist Type:</h2>
          <p className='w-full p-2 bg-slate-600 text-lg text-white font-semibold rounded-md'>{artistType}</p>
          <h2 className='text-xl text-textcolor font-semibold mt-5 mb-2'>Total Songs:</h2>
          <p className='w-full p-2 bg-slate-600 text-lg text-white font-semibold rounded-md'>{songCount}</p>
          <button
