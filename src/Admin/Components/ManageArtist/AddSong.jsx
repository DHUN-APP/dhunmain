...
//   const [songs, setSongs] = useState([{ songName: '', songId: '', coverImgUrl: '', songUrl: '' }]);
...
//         return;
//       }
//       const q = query(collection(db, 'artists'), where('name', '>=', term), where('name', '<=', term + '\uf8ff'));
...
//   const handleSuggestionClick = (artistId) => {
//     setSelectedArtistId(artistId);
//     setSuggestions([]);
//   };

//   const handleAddSong = () => {
//     setSongs([...songs, { songName: '', songId: '', coverImgUrl: '', songUrl: '' }]);
//   };
...
//   return (
//     <form className='flex flex-col max-md:w-[90%]' onSubmit={handleSubmit}>
...
//             <li
//               key={artist.id}
//               onClick={() => handleSuggestionClick(artist.id)}
...
//       {selectedArtistId && (
//         <>
//           <h2 className='text-xl text-textcolor font-semibold mt-5 mb-2 flex items-center justify-center'>Artist: {artistName}</h2>

//           {songs.map((song, index) => (
//             <div key={index} className='mb-4 flex max-md:flex-col items-center justify-center gap-3'>
//               <h2 className='text-xl text-textcolor font-semibold'>Song {index + 1}</h2>
//               <input
//                 type="text"
//                 value={song.songName}
//                 onChange={(e) => handleInputChange(index, 'songName', e.target.value)}
//                 placeholder="Song Name"
//                 className='max-md:w-full p-2 bg-slate-600 text-lg text-white font-semibold rounded-md outline-none'
//                 required
//               />
//               <input
//                 type="text"
//                 value={song.songId}
//                 onChange={(e) => handleInputChange(index, 'songId', e.target.value)}
//                 placeholder="Song ID"
//                 className='max-md:w-full p-2 bg-slate-600 text-lg text-white font-semibold rounded-md outline-none'
//                 required
//               />
...
//                 onChange={(e) => handleInputChange(index, 'coverImgUrl', e.target.value)}
...
//                 type="text"
//                 value={song.songUrl}
//                 onChange={(e) => handleInputChange(index, 'songUrl', e.target.value)}
...
const AddSong = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // Check for duplicate songIds within the new submission
      const newSongIdMap = new Map();
      const duplicateWithinNewSongs = [];
      songs.forEach((song, index) => {
        if (newSongIdMap.has(song.songId)) {
          duplicateWithinNewSongs.push(`Song ${index + 1} ID is duplicated in the submission`);
        }
        newSongIdMap.set(song.songId, true);
      });

      // Check for duplicate songIds within the existing songs
      const duplicates = [];
      songs.forEach((song, index) => {
        if (existingSongIdMap.has(song.songId)) {
          duplicates.push(`Song ${index + 1} ID already exists`);
      ...
      // Show separate toasts for each validation message
      if (duplicates.length > 0) {
  ...
  return (
    <form className='flex flex-col w-[90%]' onSubmit={handleSubmit}>
      ...
      {selectedArtistId &&  (
        <>
          <div className='flex items-center justify-center my-5'><h2 className='border-2 border-primarycolor rounded-md text-xl text-textcolor font-semibold py-2 px-5'>{artistName}</h2></div>

          {songs.map((song, index) => (
            <div key={index} className='mb-4 flex max-md:flex-col md:items-center justify-center gap-3'>
              <h2 className='text-xl text-textcolor font-semibold'>Song: {index + 1}</h2>
              <div className='flex items-center justify-center bg-slate-400  rounded-md'>
                <h3 className='max-md:w-[20%] p-2 text-slate-900 font-bold  rounded-l-md'>Name</h3>
                <input
                  type="text"
                  value={song.songName}
                  onChange={(e) => handleInputChange(index, 'songName', e.target.value)}
              ...
              <div className='flex items-center justify-center bg-slate-400  rounded-md'>
                <h3 className='max-md:w-[20%] p-2 text-slate-900 font-bold  rounded-l-md'>ID</h3>
                <input
                type="text"
                value={song.songId}
                onChange={(e) => handleInputChange(index, 'songId', e.target.value)}
              ...
              <div className='flex items-center justify-center bg-slate-400  rounded-md'>
                <h3 className='max-md:w-[20%] p-2 text-slate-900 font-bold  rounded-l-md'>Cover</h3>
                <input
                type="text"
                value={song.coverImgUrl}
                onChange={(e) => handleInputChange(index, 'coverImgUrl', e.target.value)}
              ...
              <div className='flex items-center justify-center bg-slate-400  rounded-md'>
                <h3 className='max-md:w-[20%] p-2 text-slate-900 font-bold  rounded-l-md'>File</h3>
                <input
                type="text"
                value={song.songUrl}
                onChange={(e) => handleInputChange(index, 'songUrl', e.target.value)}
