...
const LastGlobalPlaylist = ({setPlaylistId}) => {
  ...
  const { userId } = useAuth(); // Assuming you have a context to get the userId
      ...
      try {
        ...
        if (userDoc.exists()) {
          ...
          if (lastGlobalPlaylistId) {
            // Fetch the playlist document that matches the playlistId
  ...
  return (
    <div className="flex flex-col my-10 p-5 w-full">
      ...
      <div onClick={() => handlePlaylistClick(playlist.playlistId)} className="flex md:w-1/2 bg-slate-900 items-center border-2 rounded-md p-3">
        <div className=" flex">
          <img src={playlist.coverImgUrl || 'default-cover.jpg'} alt={playlist.name} className="h-32 max-md:h-24 w-32 max-md:w-24 rounded-md" />
        </div>
