...
const EditInfo = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      if (newPhoto) {
        const photoRef = ref(storage, `profilephoto/${userId}/${userId}_profilephoto.png`);
      ...
      toast.error(`Error updating document: ${error}`, { position: 'top-center' });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className='relative w-full h-screen flex flex-col items-center justify-center p-5'>
      <div className='absolute top-5 left-5 p-0 m-0'>
        <div onClick={() => navigate('/app/myprofile')} className='cursor-pointer'>
      ...
      <div className='flex flex-col items-center w-full max-w-md border-2 p-5 rounded-lg border-textcolor'>
        <div className='relative mb-5'>
          <img src={userData.photoURL} alt='Profile' className='rounded-full h-32 w-32 object-cover' />
          <label className='absolute bottom-2 right-2 cursor-pointer'>
            <BsPencilSquare size={24} className='text-white bg-blue-500 rounded-full p-1' />
