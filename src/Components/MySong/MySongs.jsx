      ...
      <div className='flex space-x-3 mt-10 max-md:mt-3 overflow-auto'>
        {filteredSongs.length > 0 ? (
          filteredSongs.map((song, index) => (
            <div key={index} onClick={() => handleClick(song.songId)} className=' flex flex-col rounded-md  border-2 max-w-24 '>
              <p className='px-2 text-md font-semibold text-textcolor'>{song.songName}</p>
              <img src={song.coverImgUrl} alt={song.songName} className='h-24 w-24 bg-slate-500' />
              <p className='px-2 text-md text-textcolor truncate'>By {song.singer}</p>
            </div>
          ))
        ) : (
