...
const Song_Details = ({ artistId, songId }) => {
  ...
  const [artistData, setArtistData] = useState(null); // Add artistData state
    ...
    const fetchSongDetails = async () => {
      try {
        ...
        if (artistDocSnapshot.exists()) {
          ...
          setArtistData(artistData); // Save artist data

          const songDetails = artistData.songs.find(song => song.songId === songId);
  ...
  if (!song) {
    return (
      <div className="flex flex-col items-center justify-center mt-10">
        <div className="text-2xl font-bold text-red-600 mb-4">Song Not Found</div>
        <p className="text-lg text-gray-600">Sorry, we couldn't find the song you're looking for.</p>
        <button 
          onClick={() => navigate('/')} 
    ...
    <div className="flex flex-col mt-10">
      ...
      <div className="flex w-full max-md:flex-col">
        <div className="w-1/5 md:p-5 flex items-center justify-center max-md:w-full">
          <img src={song.coverLink} alt={song.songName} className="h-48 w-48 rounded-md" />
        </div>
        <div className="w-4/5 p-5 max-md:w-full">
          ...
          <h2 className="text-2xl max-md:text-xl my-3 font-semibold text-textcolor">By {artistData?.name || 'Unknown Artist'}</h2>
          ...
          <div className="flex items-center text-lg font-semibold text-textcolor">
            Duration: <div className="text-sm font-normal ml-3">{duration ? `${Math.floor(duration / 60)}:${Math.floor(duration % 60).toString().padStart(2, '0')}` : 'Loading...'}</div>
          </div>
        </div>
      </div>
      <div className='mb-5'>
