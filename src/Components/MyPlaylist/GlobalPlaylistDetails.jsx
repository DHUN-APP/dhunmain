  ...
  const handleFollowToggle = async () => {
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        const playlistDoc = await transaction.get(playlistDocRef);
  
        if (!userDoc.exists() || !playlistDoc.exists()) {
        ...
        if (newFollowedStatus) {
          ...
          transaction.update(playlistDocRef, {
            followers: playlistData.followers + 1,
          });
          toast.success("Playlist followed successfully!");
        } else {
          // Unfollow the playlist
          const updatedPlaylists = userDoc.data().playlists.filter(
            (playlist) => playlist.playlistId !== playlistId
          );
          transaction.update(userDocRef, {
