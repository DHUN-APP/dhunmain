...
//       // Update user's document in Firestore with new playlist info
//       const userDocRef = doc(db, 'users', userId);
//       const userDocSnapshot = await getDoc(userDocRef);
...
//       setUploading(false);
//       toast.success('Playlist added successfully', { position: "top-center", toastId: 'playlist-toast' });
//       setPlaylistName('');
//       setCoverImg(null);
//       setSelectedSongs([]);
...
//         >
//           {songs.map((song) => (
//             <option key={song.songId} value={song.songId}><span>{song.songName}</span><span> ,{song.singer}</span></option>
//           ))}
//         </select>
//         <button
//           type="submit"
    ...
    <div className="p-5">
      ...
      <form onSubmit={handleUpload} className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-2">
          {songs.map((song) => (
            <label key={song.songId} className="flex items-center gap-2 p-2 cursor-pointer rounded-lg bg-slate-600 text-white">
              <input
                type="checkbox"
                value={song.songId}
