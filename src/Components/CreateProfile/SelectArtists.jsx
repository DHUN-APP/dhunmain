    ...
    <div className='w-full h-screen flex flex-col items-center overflow-y-scroll scrollbar-hide'>
      ...
      <ul className='w-full flex flex-wrap justify-center mb-[150px]'>
        {artists.map((artist) => (
          ...
          >
            <div className='h-[18%]'>
              <div className={`w-full flex justify-end translate-y-8 -translate-x-1 ${selectedArtists.includes(artist.id) ? '' : 'hidden'}`}>
                <UisCheckCircle size={40} color="white" />
              </div>
            </div>
            <img src={artist.photoURL} alt={artist.name} className='h-[150px] w-[150px] rounded-full border-4 shadow-inner' />
